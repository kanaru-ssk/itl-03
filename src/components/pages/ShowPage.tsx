...
// model取得
import { getUserDataByUid, getCountsByUid } from 'model/UserModel';
...
const ShowPage = () => {
	...
	useEffect(() => {
		getUserDataByUid(paramsUid).then((_user) => {
			setParamsUser(_user);
			if (!_user) setIsNotFound(true);
		});
		getCountsByUid(paramsUid).then((_counts) => {
	...
	if (!isNotFound) {
		return (
			<>
				<UserHeader paramsUserId={paramsUser?.user_id} />
				<main>
					<UserProfile paramsUser={paramsUser} paramsuserCounts={paramsuserCounts} />
					<UserButtons
						paramsUserId={paramsUser?.user_id}
